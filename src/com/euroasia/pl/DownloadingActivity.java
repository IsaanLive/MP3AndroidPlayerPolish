...
public class DownloadingActivity extends ListActivity {
    ...
    public void onCreate(Bundle icicle) {
        ...
        if(totaldownloading >= 1){
        	text.setText("Wszystkie pobierane: "+totaldownloading);
        }else{
        	text.setText("Wszystkie pobierane: 0");
        }
        ...
        this.clearall.setOnClickListener(new OnClickListener() {
        	public void onClick(View v) {
        		...
        		toastmake("Wyzeruj ukończone i nieudane pobrania");
        	}
        });
		...
		public View getView(int position, View convertView, ViewGroup parent){
	    	...
	    	if(items.get(position).third == 100){
	    		...
	    		if(a == 2){
		    		...
		    		if(a == 1){
		        		progress.setText("Pobieranie oczekujące");
		    		}else if(a == 0){
		    			progress.setText("Pobieranie ukończone");
		    		}else if(a == -1){
		        		progress.setText("Nieudane pobieranie");
		    		}
	    		}
	...
	protected void onListItemClick(ListView l, View v, int position, final long id) {
		...
		if(items.get(position).third != 100){
			...
			if(downloaded == -1){
				...
				builder.setMessage("Sposób pobierania")
				       .setCancelable(false)
				       .setNegativeButton("Usuń", new DialogInterface.OnClickListener() {
				           public void onClick(DialogInterface dialog, int id) {
				                mDbHelper.deleteDownload(ida);
				                refreshlist();
				                toastmake("Usunięto pobieranie MP3");
				           }
				       })
				       .setNeutralButton("Ponów", new DialogInterface.OnClickListener() {
				           public void onClick(DialogInterface dialog, int id) {
				        	   mDbHelper.updateDownloaded(ida, "1"); 
				        	   refreshlist();
				        	   toastmake("Ustaw na ponawianie");
				           }
				       })
				...
				final CharSequence[] items = {"Otwórz MP3", "Ustaw jako dzwonek", "Pobierz ponownie", "Usuń", getString(R.string.btn_cancel_name)};
	
				AlertDialog.Builder builder = new AlertDialog.Builder(this);
				builder.setTitle("Wybierz sposób");
				builder.setItems(items, new DialogInterface.OnClickListener() {
				    public void onClick(DialogInterface dialog, int item) {
				    	if(items[item] == "Otwórz MP3"){
				    	    try {
				    	        //launch intent
				    	    	...
				    	    	toastmake("Błąd przy pobieraniu MP3");
				    	        e.printStackTrace();
				    	    }
				    	}else if(items[item] == "Ustaw jako dzwonek"){
							...
							builder.setMessage("Czy na pewno chcesz usunąć: " + title + " ?")
							       ...
							       .setPositiveButton("Tak", new DialogInterface.OnClickListener() {
							           public void onClick(DialogInterface dialog, int id) {
							        	    ...
							        	    mDbHelper.deleteDownload(ida);
							        	    refreshlist();
							                toastmake("Usunięto MP3");
							           }
							       })
							       .setNegativeButton("Nie", new DialogInterface.OnClickListener() {
							           public void onClick(DialogInterface dialog, int id) {
							        	   toastmake("Nie usunięto MP3");
							               dialog.cancel();
							           }
							       });
							AlertDialog alert = builder.create();
							alert.show();
				    	}else if(items[item] == "Pobierz ponownie"){
				    		mDbHelper.updateDownloaded(ida, "1"); 
				    		refreshlist();
				    		toastmake("Ustaw na ponowienie");
				    	}
				    }
				});
				AlertDialog alert = builder.create();
				...
				builder.setMessage("Sposób pobierania")
				       .setCancelable(false)
				       .setNegativeButton("Zatrzymaj pobieranie", new DialogInterface.OnClickListener() {
				           public void onClick(DialogInterface dialog, int id) {
				        	   if(sid == 1){
				                ...
				                toastmake("Zatrzymano pobieranie MP3");
				           }
				       })
				       .setPositiveButton(getString(R.string.btn_cancel_name), new DialogInterface.OnClickListener() {
				           public void onClick(DialogInterface dialog, int id) {
    ...
    public void refreshlist(){
    	...
    	if(a.getCount() != 0){
    		adapter.addAdapterItem(new AdapterItem(102109, "Obecnie pobierane", 100));
    		a.moveToFirst();
    	...
    	if(a1.getCount() != 0){
    		adapter.addAdapterItem(new AdapterItem(102110, "Oczekujące pobieranie", 100));
    	...
    	if(a11.getCount() != 0){
    		adapter.addAdapterItem(new AdapterItem(102111, "Pobrane", 100));
    	...
    	if(a111.getCount() != 0){
    		adapter.addAdapterItem(new AdapterItem(102112, "Nie udało się pobrać", 100));
