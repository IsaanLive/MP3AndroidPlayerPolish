...
public class SearchActivity extends ListActivity {
    ...
    public void onCreate(Bundle savedInstanceState) {
		...
		headerview.setText("Wyszukiwanie");
    ...
    public void startsearch(){
		...
		toastmake("Szukanie: "+searchstring);
	...
	private class RetrieveSearch extends AsyncTask<Void, Void, Void>{
		...
		protected void onCancelled() {		
			...
			TextView failresults = (TextView)findViewById(R.id.failresults);
			failresults.setText("Anulowano wyszukiwanie");
		...
		protected void onPreExecute() {
			...
			cancelDialog.setMessage("Wyszukiwanie utworów...");
		...
		protected void onPostExecute(Void result) {
			if(ia == 0 && fail == 0){
		        ...
		        if(!isInternetConnectionActive(getApplicationContext())) {
					failresults.setText("Wygląda na to, że nie posiadasz działającego połączenia internetowego, lub sygnał połączenia jest słaby. Uruchom sieć lub włącz przekaz danych i spróbuj ponownie.");
		        }else{
					failresults.setText("Nie zwrócono żadnych wyników");
		        }
			}else if(ia == 0 && fail > 0){
				fail = 0;
		        ...
		        if(!isInternetConnectionActive(getApplicationContext())) {
					...
					failresults.setText("Wystąpił błąd w trakcie wyszukiwania. Spróbuj ponownie");
		        }
			}else{
				setListAdapter(adapter);
			}
		...
		public void getDilandau(String search) throws IOException, MalformedURLException, NullPointerException{
			...
			if(!sb.toString().contains("Nie znaleziono utworów")){
